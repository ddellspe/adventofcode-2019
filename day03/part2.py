...
def calculate_wire_distance(grid, row_origin, col_origin, path_1, path_2):
  p1_distances = []
  p1_locations = []
  p2_distances = []
  p2_locations = []
  ...
  for move in path_1:
    ...
    if dir == 'R':
      for step in range(amount):
        ...
        distance+=1
        if grid[row][col] == 'X':
          p1_distances.append(distance)
          p1_locations.append((row, col))
    elif dir == 'L':
      for step in range(amount):
        col-=1
        distance+=1
  ...
  for move in path_2:
    ...
    if dir == 'R':
      for step in range(amount):
        ...
        distance+=1
        if grid[row][col] == 'X':
          p2_distances.append(distance)
          p2_locations.append((row, col))
    elif dir == 'L':
      for step in range(amount):
        col-=1
        distance+=1
      ...
      for step in range(amount):
        ...
        if grid[row][col] == 'X':
          ...
          p2_locations.append((row, col))
  distances = []
  for ind, loc in enumerate(p1_locations):
    for ind2, loc2 in enumerate(p2_locations):
      if loc == loc2:
        distances.append(p1_distances[ind]+p2_distances[ind2])
...
wire_distance = calculate_wire_distance(full_grid, row_origin, col_origin, wire_1_path, wire_2_path)
print('Distance is: ' + str(distance))
print('Wire Distances is: ' + str(wire_distance))