...
def calculate_ore(fuel_count, reaction_map, ordered_dependents):
    ...
    while len(ingredients) > 1:
        ...
        for dependent in ordered_dependents:
            for ingredient in ingredients:
                if dependent == ingredient[1]:
                    ...
                    if generated > ingredient[0]:
                        extras.append((generated-ingredient[0], ingredient[1]))
        ...
        extras = compact_list(extras)
    return ingredients[0][0], extras
...
fuel_count = 1
ore, extras = calculate_ore(fuel_count, reaction_map, ordered_dependents)
extra_min = len(extras)
# fuel_count = math.floor(1000000000000/ore)
while ore < 1000000000000:
    fuel_count+=1
    ore, extras = calculate_ore(fuel_count, reaction_map, ordered_dependents)
    if len(extras) < extra_min:
        extra_min = len(extras)
        fuel_count = math.floor(1000000000000/ore*fuel_count)-1000
print(fuel_count-1)